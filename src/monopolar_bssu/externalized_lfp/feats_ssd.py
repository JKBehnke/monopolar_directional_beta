...
import meet as meet  # https://github.com/neurophysics/meet
...
def get_SSD_component(
    ...
    SSD_pattern /= np.abs(SSD_pattern).max(-1)[:,np.newaxis] # divide SSD_pattern by 
    ...
    # if multiple components are asked
        ...
        for i, c in enumerate(return_comp_n): # loop only over number of components of interest
            return_array[i, :] = SSD_filtered_data[c]
        return return_array, SSD_pattern, SSD_eigvals # SSD_pattern and SSD_eigvals -> all 
